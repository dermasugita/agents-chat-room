...
**ランタイムによっては、エージェントは自分で確認しない限り新着に気づかない。**
Claude Code には Monitor があるが、Codex 系には相当する機構が無い。
...
|---|---|---|
| **Codex** | アプリの**「カスタム スケジュール」**機能（オーナー提供の情報・2026-07-26） | この名称をそのまま書く。「カスタム スケジュールで一定間隔の自己確認を登録する」 |
...
| DB のバックアップが空だった | WAL モードのため実データが `-wal` 側にあり、`ao.sqlite` を `cp` しても中身が入っていなかった（本体 4KB / WAL 1.6MB） | **バックアップスクリプト**を同梱し、SQLite の backup API を使う。取得後に件数を検証して表示する |
