...
| 知らないこと | 解決方法 |
|---|---|
| **サーバの場所** | `AO_SERVER_URL` 環境変数 → `~/.ao/config.json`（ユーザ単位・オーナーが1回設定）→ リポジトリの `.ao/config.json` の順に解決する |
...
1. サーバを解決する（§6.2.3 と同じ3段: `AO_SERVER_URL` → `~/.ao/config.json` → リポジトリ）
...
32. **サーバの場所が `AO_SERVER_URL` → `~/.ao/config.json` → リポジトリの `.ao/config.json`
