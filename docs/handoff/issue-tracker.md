...
> ⚠️ **このプロジェクトでは、設計文書の作業用の正は git リポジトリの `docs/` である。**
> `.ao/docs/` の写しは 2026-07-26 06:43 の取り込み時点で止まっており、
> **約60コミット分古い**（例: サービス側の ADR 0006 のタイトルは撤回済みの
> 「心拍はポーリングの副産物とする」のまま。ADR 0009 と 0010 は存在しない）。
>
> **`docs/adr/` と `docs/handoff/` をリポジトリから読むこと。** `.ao/docs/` を信じないこと。
>
> これは [ADR 0001](../adr/0001-documents-source-of-truth-in-db.md)（DB が正本）に
> 反する状態であり、設計者の運用の誤りである。
> 文書の削除 API が無いため即座に直せない（基準69・70）。
...
- **添付ファイル**
- **外部への通知**（メール・Slack・push）

  > **訂正（2026-07-26、オーナー指示）**: 当初ここに
  > 「課題は自分で見に行くもの」と書いていたが、**撤回した。**
  > 課題は `watch` の出力に自動で載る（基準82、[ADR 0011](../adr/0011-issue-tracker-outside-the-ball-mechanism.md) の追記）。
  > 持たないのは**外部サービスへの通知**であって、サービス内での配達は行う。
  > 配達（目に入る）と義務（応答しないと異常として上がる）は別である。
...
7. **課題の操作は心拍を更新しない。**
   課題は他プロジェクトへも投稿できるため、投稿先で心拍を打つと
   **その参加者でもない相手のプロジェクトに幽霊の参加者行ができる。**
   エージェントの心拍は `ao watch --once` 側で維持されるので、
   課題操作が打たなくても不足しない
   （実装者の判断を設計者が承認して規約化した。2026-07-26）
...
- **`question` と課題の使い分け**: すぐ応答が必要なら `question`、
  いつか誰かが見るべきなら課題
- **新しい課題と状態変化は `ao watch --once` の `ISSUE` 行で届く。**
  探しに行かなくてよい。ただしボールは作らないので、
  未対応のまま放置されていても離脱扱いにはならない

`design-handoff`（設計者）に足すこと。

- **起動時に1回だけ**、担当プロジェクトの未対応課題を全件見る
- 以後は `ao watch --project --once` の `ISSUE` 行で届く。
  **周期ループに `ao issues` を入れない**

> **訂正（2026-07-26、オーナー指示）**: 当初ここに「課題には通知が来ない。
> 自分で見に行く」「見に行かなければ気づかない」と書いていたが、**撤回した。**
> 基準82 で `watch` の出力に載せる。
>
> 「見に行くことを覚えていないと機能しない」仕組みは、このプロジェクトが
> 繰り返し落ちてきた穴である。心拍を能動コマンドに紐づけたのも、
> worktree とスケジューラを番号付きの必須手順にしたのも、
> **「スキルに書いたのに実行されなかった」を潰すため**だった。
> 課題だけ例外にする理由がない。
